...
export default function RiskProfilingComponent() {
    ...
    const loadIndustryOptions = (searchValue, callback) => {
        const filtered = industries.filter((i) =>
            i.value.toLowerCase().includes(searchValue.toLowerCase())
        );
        callback(filtered);
    };

    const loadOccupationsOptions = (searchValue, callback) => {
        const filtered = occupations.filter((i) =>
            i.value.toLowerCase().includes(searchValue.toLowerCase())
        );
        callback(filtered);
    };
    ...
    const loadCustomerTypesOptions = (searchValue, callback) => {
        const filtered = customerTypes.filter((i) =>
            i.value.toLowerCase().includes(searchValue.toLowerCase())
        );
        callback(filtered);
    };
    ...
    const loadSanctionsOptions = (searchValue, callback) => {
        const filtered = sanctions.filter((i) =>
            i.value.toLowerCase().includes(searchValue.toLowerCase())
        );
        callback(filtered);
    };
    ...
    const loadRegulatoriesOptions = (searchValue, callback) => {
        const filtered = regulatory.filter((i) =>
            i.value.toLowerCase().includes(searchValue.toLowerCase())
        );
        callback(filtered);
    };
    ...
    const handleWeightAlter = (selectedOption) => {
        setSelectedOptions((prev) => [
            ...prev,
            { value: selectedOption.value, weight: selectedOption.weight }
        ]);
    };
    ...
    const handleGenerateWeight = (event) => {
        ...
        const average = count > 0 ? weightCount / count : 0;

        // determine risk rating
        let riskLevel = "Low";
        if (average >= 3) riskLevel = "High";
        else if (average >= 2) riskLevel = "Medium";

        // return 0,1 or multiple results depending on logic
        // If you want real API integration later, replace this.
        const generatedResults = [
            {
                name: "Client Profile",
                risk: riskLevel,
                list: "Internal Risk Model",
                confidence: `${Math.round(average * 25)}%`,
                status: average === 0 ? "No Match Found" : "Profile Scored"
            }
        ];
        ...
        <div className={styles.container}>
            <form className={styles.form} onSubmit={handleGenerateWeight}>
                ...
                <div className={styles.formGrid}>
                    ...
                    <div className={styles.formGroup}>
                        <label>PEP / Sanction Status</label>
                        <AsyncSelect
                    ...
                    <div className={styles.formGroup}>
                        <label>Regulatory / Adverse Media</label>
                        <AsyncSelect
            ...
            <div className={styles.results}>
                <div className={styles.resultsHeader}>
                    ...
                    <i>{results.length} results found</i>
                </div>
                ...
                <table>
                    ...
                    <tbody>
                        {results.length === 0 ? (
                            <tr>
                                <td colSpan="6" className={styles.noMatch}>
                                    No match found
                                </td>
                            </tr>
                        ) : (
                            results.map((r, index) => (
                                <tr key={index}>
                                    ...
                                    <td className={
                                        r.status === "No Match Found"
                                            ? styles.statusClear
                                            : r.status === "Profile Scored"
                                                ? styles.statusMatch
                                    ...
                                    <td className={styles.viewAction}>View</td>
                                </tr>
                            ))
                        )}
                    </tbody>
